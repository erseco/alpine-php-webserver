...
`REAL_IP_HEADER`, `REAL_IP_RECURSIVE`, and `REAL_IP_FROM` restore the real client IP from a trusted reverse proxy chain. This is separate from proxy HTTPS handling like `X-Forwarded-Proto`.
...
Only trust proxy IPs or CIDRs that are under your control. Do **not** set `REAL_IP_FROM` to broad public ranges such as `0.0.0.0/0`, because that would allow clients to spoof forwarded IP headers.
